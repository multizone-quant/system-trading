...
class MainTable(tk.Canvas):
    def __init__(self,
        ...
        self.my_texts = []
        self.my_high_texts = []
        self.my_high_col = [] # hightlight row, col
        self.repeat = 0

    def is_highlight_cell(self, r, c) :
        for each in self.my_high_col :
            if each[0] == r and each[1] == c :
                return 1
        return 0

    def find_t(self, texts, x, y) :
        for each in texts :
            if each['x'] == x and each['y'] == y :
                return each['text']
        return ''

    def get_my_text(self, texts, x, y) :
        t = self.find_t(texts, x, y)
        return t
...
#        t = self.get_high_t(x1, y1, x2, y2, fill, outline, tag, can_width)
#        if 0:
        if self.hidd_high:
            t, sh = self.hidd_high.popitem()
            new_t = self.get_my_text(self.my_high_texts, x1, y1)
            t = new_t
            ...
            self.my_high_texts.append({'x':x1, 'y':y1, 'text':t, 'content':'123'})
    ...
    def main_table_redraw_grid_and_text(self, redraw_header = False, redraw_row_index = False, redraw_table = True):
        ...
        if redraw_table:
            for c in range(start_col, end_col - 1):
                for r in rows_:
                    is_higglighted = self.is_highlight_cell(r, c)
                        ...
                        if cell_alignment == "w":
                            ...
                            if y + self.half_txt_h - 1 > y1:
                                ...
                                if self.hidd_text:
                                    ...
                                    new_t = self.get_my_text(self.my_texts, x, y)
                                    t = new_t
                                    self.coords(t, x, y)
                                    if sh:
                                    ...
                                    self.my_texts.append({'x':x, 'y':y, 'text':t, 'content':txt})
                                ...
                                if y + self.half_txt_h - 1 < sr:
                                    for i in range(stl, len(lns)):
                                        ...
                                        if self.hidd_text:
                                            ...
                                            t = self.create_text(x, y, text = txt, fill = tf, font = self.my_font, anchor = "w", tag = "t")
                                            self.my_texts.append({'x':x, 'y':y, 'text':t, 'label':txt})
                            ...
                            if y + self.half_txt_h - 1 > y1:
                                ...
                                if self.hidd_text:
                                    ...
                                    # todo : background color가 있는 경우만
                                    # self.hidd_text.update({t:True})
                                    if is_higglighted :
                                        self.tag_raise("t")
                                    else :
                                        a = 1
